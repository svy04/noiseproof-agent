...
Latest proof routing now points reviewers to the retrieval run semantic provenance runtime proof at `docs/review/retrieval-run-semantic-provenance-runtime-smoke.md`, plus `docs/review/retrieval-run-semantic-provenance-runtime-smoke-remote-verification.md` and `docs/review/external-review-issue-body-retrieval-run-semantic-provenance-runtime-refresh.md`. This is reviewer routing only; it is not external reviewer feedback, not hosted deployment evidence, not semantic retrieval quality evidence, not embedding generation, and not product-complete.
