...
- Child `workflow_run_id` columns, semantic retrieval, embeddings, LLM calls, external search, and free-form final answer generation: not implemented
...
After Deterministic Workflow Execution Preview v0, the next phase should improve workflow lineage carefully without overstating the runtime:

- review whether the deterministic preview gives enough runtime evidence to justify nullable child `workflow_run_id` schema changes
