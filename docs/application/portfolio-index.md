...
Failure cases are not linked to workflow parents yet. The current failure-case workflow linkage review keeps `workflow_run_id` on failure cases deferred until the system has a real failure-case creation path from failed workflow parents. `POST /failure-cases/draft-preview` can prepare a human-confirmed draft payload from workflow failure evidence, and the route-level smoke verification records that this preview keeps `failure_cases` unchanged. A separate manual handoff smoke shows that a human-confirmed draft can be submitted to existing `POST /failure-cases`, and the fresh DB handoff smoke verifies the same path against a local migrated PostgreSQL database. It does not persist that payload or create failure cases automatically.
