...
- Upload path: raw upload quarantine metadata, guarded raw file download endpoint runtime smoke, and local v0 download rate limiting are locally smoke-tested; production authorization, distributed rate limits, production malware evidence, robust PDF, and hosted evidence remain unproven.
