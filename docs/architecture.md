...
- Agent-run Linkage Review v0 exists as a review artifact; direct `agent_run_id` foreign-key linkage is not implemented.
...
- Web app, file upload parsing, robust PDF extraction, persisted chunks, persisted collection plans, embeddings, child-record `agent_run_id` foreign-key linkage for Evidence Ledger / Noise Gate / Report records, distributed tracing, hosted observability, and agents are planned but not implemented.
