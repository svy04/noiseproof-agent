...
Phase marker: external feedback current-state Gate/Report semantic source provenance issue verification v0.

## Purpose

Screen issue #1 after the owner-authored Gate/Report semantic source provenance issue-body refresh.

This verifies that the live issue routes reviewers to the latest Gate/Report proof chain while keeping external reviewer feedback v0 pending unless a qualifying outside comment exists.
...
```text
remote verification for this current-state Gate/Report issue screen after push, external reviewer feedback v0 if qualifying outside feedback exists, owner-runtime manual live embedding smoke v0 only when OPENAI_API_KEY is configured by the owner, or another source-first product gate selected from docs/GOAL.md
```
