...
Phase marker: external reviewer extension-allowlist request refresh v0.

## Purpose

Make the uploaded raw file extension allowlist runtime smoke discoverable from reviewer-facing repository paths.
...
```text
allowed CSV upload -> 201
extension boundary warnings are present
sample.exe.csv double-extension block -> 415
blocked response has no raw bytes
blocked content hash is not present in recent raw uploads
...
```text
external review issue body extension-allowlist refresh v0, external reviewer feedback v0 if qualifying outside feedback exists, or another source-first product gate selected from docs/GOAL.md
```
