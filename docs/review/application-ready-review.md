...
Status: Phase 33.5 review packet.
...
| missing manifest reference behavior has a bounded next proof step | Partial / reviewed | `docs/review/workflow-lineage-missing-reference-review.md` selects a targeted missing-reference test before schema or mutation changes | behavior exists in the read model, but broken-reference fixture is not yet tested |
...
```text
NoiseProof Agent is a small, inspectable portfolio service that shows how messy market data can be profiled, retrieved, converted into persisted evidence entries, persisted as gate decisions, stored as claim-bounded report preview records, linked to a deterministic workflow parent with `workflow_run_id` plus `workflow_trace_id` correlation, inspected from the workflow parent detail endpoint, annotated with local stage input manifests for deterministic gate/report stages, projected through a derived workflow lineage read model, reached from dashboard workflow-row links, and reviewed for missing manifest reference boundaries. It does not yet claim direct evidence -> gate -> report foreign-key lineage.
```
