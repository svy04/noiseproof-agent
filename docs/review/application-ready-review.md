...
| failed workflow stage events are recorded | Pass for deterministic workflow preview failures | `POST /workflow-runs/execute-preview` records the active failed stage in `workflow_stage_events`; `docs/review/workflow-failed-stage-event.md`; `test_workflow_execute_preview_records_failed_stage_event_when_stage_errors` | local failure observability only; not retry behavior, automatic failure-case creation, root-cause automation, distributed tracing, hosted observability, or product-complete |
