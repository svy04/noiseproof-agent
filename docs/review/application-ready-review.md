...
Status: Phase 30.5 review packet.
...
| direct stage-level input links exist | Fail / deliberately deferred | `docs/review/direct-evidence-gate-report-cross-link-review.md` | downstream stages do not yet consume persisted upstream row ids |
...
```text
NoiseProof Agent is a small, inspectable portfolio service that shows how messy market data can be profiled, retrieved, converted into persisted evidence entries, persisted as gate decisions, stored as claim-bounded report preview records, linked to a deterministic workflow parent with `workflow_run_id` plus `workflow_trace_id` correlation, and inspected from the workflow parent detail endpoint. It does not yet claim direct evidence -> gate -> report foreign-key lineage.
```
