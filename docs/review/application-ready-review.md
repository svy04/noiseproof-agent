...
| workflow failure-case persistence handoff exists | Pass | `POST /failure-cases/workflow-runs/{workflow_run_id}`, `docs/review/workflow-failure-case-persistence-handoff.md` | caller-triggered `caller_triggered_workflow_failure_case_persistence`; not background automation, root-cause automation, or complete workflow failure causality |
