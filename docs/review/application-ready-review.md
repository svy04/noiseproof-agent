...
| workflow stage event log exists | Pass for deterministic workflow preview | `workflow_stage_events`; `db/migrations/024_workflow_stage_events.sql`; `GET /workflow-runs/{id}` and `GET /workflow-runs/{id}/proof-bundle` expose `stage_events` with `workflow_stage_event_count`; `docs/review/workflow-stage-event-log.md`; `docs/review/workflow-stage-event-log-runtime-smoke.md` | local event log only; not distributed tracing, OpenTelemetry, hosted observability, autonomous workflow execution, or product-complete |
