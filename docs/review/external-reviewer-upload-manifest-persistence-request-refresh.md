# External Reviewer Upload-manifest Persistence Request Refresh

Phase marker: external reviewer upload-manifest persistence request refresh v0.

## Purpose

This gate updates the external reviewer request path so a reviewer can inspect the uploaded-file intake manifest persistence proof without reading the full phase history.
...
```text
The service can persist and list upload intake manifest metadata through POST /documents/upload-intake-manifests and GET /documents/upload-intake-manifests. It keeps storage_decision = do_not_persist_raw_upload_yet and persistence_boundary = manifest_only_no_raw_file_storage.
```
...
The next bounded gate should update or verify the live public issue body so issue #1 points reviewers to the uploaded-file intake manifest persistence proof:

```text
external reviewer upload-manifest persistence issue-body refresh v0
```
