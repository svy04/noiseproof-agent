...
Phase 862, the gap has a toy qrels-backed evaluation, a live lexical baseline
generated by the existing `retrieve_candidates()` path, and a caller-provided
vector live semantic baseline over the same tiny fixture, but still not semantic
retrieval quality evidence:

```text
status -> unproven
claim_boundary -> caller_provided_live_semantic_baseline_and_toy_qrels_do_not_prove_semantic_retrieval_quality
...
proof_routes -> docs/evaluation/live-semantic-qrels-baseline-report.md; docs/evaluation/live-lexical-qrels-baseline-report.md; docs/evaluation/qrels-backed-semantic-quality-report.md
```
