...
Record the current issue #1 state after the workflow failed stage event runtime issue body refresh, and keep the external reviewer feedback gate pending because the only comment is still self-authored.
