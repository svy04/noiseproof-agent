...
- Boundary: local Docker PostgreSQL plus live FastAPI HTTP evidence for `GET /workflow-runs/{id}/proof-bundle`, dashboard navigation from `GET /ops/dashboard` to the `proof bundle` link, linked failure-case visibility through `GET /workflow-runs/{id}`, `GET /workflow-runs/{id}/proof-bundle`, and `GET /failure-cases?workflow_run_id={id}` with unrelated failure cases filtered out, dashboard failure-case count/filter links with `dashboard_contains_linked_failure_case_filter: true` and `dashboard_omits_unlinked_failure_case_filter: true`, dashboard anchor inspection with `data-method="GET"`, `extracted_anchor_count: 38`, `unique_anchor_count: 25`, and every unique href returning GET 200, local Playwright browser automation with `browser_anchor_count: 27`, `browser_get_anchor_count: 27`, `browser_post_anchor_count: 0`, and `post_only_draft_preview_anchor_count: 0`, direct workflow-created stage links with `direct_stage_link_count: 3`, `evidence_to_report`, `evidence_to_noise_gate`, `noise_gate_to_report`, and `workflow_created_records_only_not_standalone_payload_lineage`, workflow stage event rows with `detail_stage_event_count: 4`, `bundle_stage_event_count: 4`, `retrieval,evidence_ledger,noise_gate,report`, and `local_workflow_stage_event_log_not_distributed_tracing`, plus failed stage event runtime evidence with `POST /workflow-runs/execute-preview -> 500`, `workflow_stage_event_count: 2`, `retrieval -> completed`, `evidence_ledger -> failed`, `failure_case_count_delta -> 0`, and `local_workflow_stage_failure_event_no_retry_no_auto_failure_case`; not a live issue body edit, not external reviewer feedback, not hosted deployment evidence, not customer validation, not retry behavior, not automatic failure-case creation, not distributed tracing, not hosted observability, not autonomous workflow execution, not automatic failure detection, not background automation, not complete workflow failure causality, and not product-complete.
