...
CLAIM_BOUNDARY = (
    "source_policy_no_native_text_ocr_owner_rights_decision_record_not_clearance"
)
...
def build_source_policy_no_native_text_ocr_owner_rights_decision_record(
    source_rights_review_request_packet: dict[str, Any],
) -> dict[str, Any]:
    _validate_source_rights_review_request_packet(
        source_rights_review_request_packet
    )
    return {
        "record": PHASE_MARKER,
        ...
        "target_fixture_id": source_rights_review_request_packet[
            "target_fixture_id"
        ],
        "publisher": source_rights_review_request_packet["publisher"],
        "source_url": source_rights_review_request_packet["source_url"],
        "source_policy_url": source_rights_review_request_packet[
            "source_policy_url"
        ],
        "source_sha256": source_rights_review_request_packet["source_sha256"],
        "source_sha256_algorithm": "sha256",
        "source_rights_review_request_packet_phase_marker": (
            source_rights_review_request_packet["phase_marker"]
        ),
        ...
        "minimum_next_artifact": "source rights request delivery record",
...
def load_source_policy_no_native_text_ocr_owner_rights_decision_record(
    path: Path | str,
) -> dict[str, Any]:
...
def build_source_policy_no_native_text_ocr_owner_rights_decision_record_summary(
    record: dict[str, Any],
) -> dict[str, Any]:
    validate_source_policy_no_native_text_ocr_owner_rights_decision_record(
        record
    ...
    return {
        ...
        "source_rights_review_request_packet_phase_marker": record[
            "source_rights_review_request_packet_phase_marker"
        ],
        ...
        "owner_rights_decision_recorded": record[
            "owner_rights_decision_recorded"
        ],
        "source_rights_request_delivery_needed": record[
            "source_rights_request_delivery_needed"
        ],
        ...
        "can_claim_owner_rights_decision_record": record[
            "can_claim_owner_rights_decision_record"
        ],
...
def build_source_policy_no_native_text_ocr_owner_rights_decision_record_report(
    ...
    lines = [
        ...
        f"owner_decision -> {summary['owner_decision']}",
        ...
        "source_rights_owner_decision_recorded -> "
        + _format_bool(summary["source_rights_owner_decision_recorded"]),
        "owner_rights_decision_recorded -> "
        + _format_bool(summary["owner_rights_decision_recorded"]),
        "source_rights_request_delivery_needed -> "
        + _format_bool(summary["source_rights_request_delivery_needed"]),
        "source_rights_request_delivery_performed -> "
        ...
        "## Decision Basis",
        "",
        *[f"- {item}" for item in summary["decision_basis"]],
        "",
        "## Blocked Actions",
        "",
        *[f"- {item}" for item in summary["blocked_actions"]],
        ...
        "## Allowed Actions",
        "",
        *[f"- {item}" for item in summary["allowed_actions"]],
        "",
        "## Claim Boundary",
        "",
        f"can_claim_owner_rights_decision_record -> {_format_bool(summary['can_claim_owner_rights_decision_record'])}",
...
def validate_source_policy_no_native_text_ocr_owner_rights_decision_record(
    ...
    expected_values = {
        ...
        "source_sha256_algorithm": "sha256",
        "source_rights_review_request_packet_phase_marker": PREVIOUS_GATE,
        "decision_scope": "repository_transcript_and_hash_commit_boundary",
        "owner_decision": (
            "do_not_collect_or_commit_transcript_until_source_rights_response_exists"
        ),
    ...
    for field, expected in expected_values.items():
        if payload.get(field) != expected:
            raise ValueError(
                f"source-policy OCR owner rights decision record {field} must be {expected!r}"
            )
    ...
    for field in ["warnings", "boundary_notes"]:
        ...
        if not isinstance(value, list) or not value:
            raise ValueError(
                f"source-policy OCR owner rights decision record {field} must be a non-empty list"
            )

    for note in BOUNDARY_NOTES:
        if note not in payload["boundary_notes"]:
            raise ValueError(
                f"source-policy OCR owner rights decision record missing boundary note: {note}"
            )


def _validate_source_rights_review_request_packet(payload: dict[str, Any]) -> None:
    expected_values = {
    ...
    for field, expected in expected_values.items():
        if payload.get(field) != expected:
            raise ValueError(
                f"source rights review request packet {field} must be {expected!r}"
            )
    ...
    if isinstance(value, dict):
        for key, nested in value.items():
            if key in _RAW_TEXT_FIELDS:
                raise ValueError(
                    f"source-policy OCR owner rights decision record must not commit {key}"
                )
