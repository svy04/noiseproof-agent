...
def probe_tesseract_dependency(
    *,
    ...
    languages = _parse_languages(str(getattr(language_result, "stdout", ""))) if language_ok else []
    ...
    return {
        **_base_probe_result(
        ...
        "tesseract_version_reported": bool(_parse_version(str(getattr(version_result, "stdout", "")))),
        "tesseract_version": _parse_version(str(getattr(version_result, "stdout", ""))),
...
def _parse_version(output: str) -> str | None:
    ...
    match = re.search(r"tesseract\s+([0-9][^\s]*)", first_line, re.IGNORECASE)
