...
Phase 33 adds workflow row links in `GET /ops/dashboard` to both `GET /workflow-runs/{id}` and `GET /workflow-runs/{id}/lineage`. Phase 34 adds a targeted missing-reference fixture for the derived lineage read model. Phase 35 hardens manifest-shape handling for non-list evidence id values. These phases do not add dashboard polish, a frontend framework, malformed-manifest mutation endpoint, or new lineage storage.
