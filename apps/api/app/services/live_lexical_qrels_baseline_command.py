...
from packages.ingestion.retrieval.live_lexical_qrels import (
    BOUNDARY,
    build_live_lexical_qrels_baseline_report,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Regenerate the live lexical qrels baseline report."
    )
    parser.add_argument(
        "--fixture-root",
        required=True,
    ...
    parser.add_argument(
        ...
        help="Existing lexical retrieval chunk strategy to evaluate.",
    )
    ...
    try:
        ...
        evaluation = evaluate_live_lexical_qrels_baseline(
            fixture,
            strategy=args.strategy,
            k=args.k,
        )
        report = build_live_lexical_qrels_baseline_report(evaluation)
    except (OSError, ValueError) as exc:
        print("live_lexical_qrels_baseline_report_failed", file=sys.stderr)
    ...
    if args.check:
        try:
            ...
            return 2
        if current != report:
            print("live_lexical_qrels_baseline_report_stale", file=sys.stderr)
            print("byte-for-byte regeneration mismatch", file=sys.stderr)
            print("not semantic retrieval quality evidence", file=sys.stderr)
            return 3
        print("live_lexical_qrels_baseline_report_current")
    ...
    print("live lexical qrels baseline v0")
    print(BOUNDARY)
    print("not semantic retrieval quality evidence")
    print(str(output))
    return 0
