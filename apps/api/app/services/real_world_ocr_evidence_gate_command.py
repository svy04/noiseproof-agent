...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Regenerate the real-world OCR evidence gate report."
    )
    parser.add_argument("--evidence", required=True, help="OCR evidence JSON path.")
    ...
    try:
        evidence = load_real_world_ocr_evidence(Path(args.evidence))
        summary = build_real_world_ocr_evidence_summary(evidence)
        report = build_real_world_ocr_evidence_report(summary)
    except (OSError, ValueError) as exc:
        print("real_world_ocr_evidence_gate_report_failed", file=sys.stderr)
    ...
    if args.check:
        try:
            ...
            return 2
        if current != report:
            print("real_world_ocr_evidence_gate_report_stale", file=sys.stderr)
            print("byte-for-byte regeneration mismatch", file=sys.stderr)
            print("not robust PDF extraction evidence", file=sys.stderr)
            return 3
        print("real_world_ocr_evidence_gate_report_current")
        print(f"ocr_gate_status={summary['ocr_gate_status']}")
    ...
    print("real_world_ocr_evidence_gate_v0")
    print(f"ocr_gate_status={summary['ocr_gate_status']}")
    print("not robust PDF extraction evidence")
    print(str(output))
    return 0
