...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args and args[0] == "--from-source-rights-review-request-packet":
        try:
            packet_path, record_path = _parse_generation_args(args)
        ...
        print("source_policy_no_native_text_ocr_owner_rights_decision_record_written")
        print(record_path)
        return 0
    ...
    if check:
        ...
        if current != report:
            print("source_policy_no_native_text_ocr_owner_rights_decision_record_report_stale")
            return 1
        print("source_policy_no_native_text_ocr_owner_rights_decision_record_report_current")
    else:
        output_path.write_text(report, encoding="utf-8")
        print("source_policy_no_native_text_ocr_owner_rights_decision_record_report_written")
    print(
        "can_claim_owner_rights_decision_record="
        + _format_bool(summary["can_claim_owner_rights_decision_record"])
    )
    print(
...
def _parse_generation_args(args: Sequence[str]) -> tuple[Path, Path]:
    if (
        len(args) != 4
        or args[0] != "--from-source-rights-review-request-packet"
        or args[2] != "--output-owner-rights-decision-record"
    ):
        raise ValueError(
            "expected --from-source-rights-review-request-packet <path> --output-owner-rights-decision-record <path>"
        )
...
def _parse_report_args(args: Sequence[str]) -> tuple[Path, Path, bool]:
    ...
    if (
        len(values) != 4
        or values[0] != "--owner-rights-decision-record"
        or values[2] != "--output"
    ):
        raise ValueError(
            "expected --owner-rights-decision-record <path> --output <path> [--check]"
        )
