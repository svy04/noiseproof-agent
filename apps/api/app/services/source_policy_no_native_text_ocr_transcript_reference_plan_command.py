...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args and args[0] == "--from-marker-proxy-eval":
        try:
            marker_proxy_eval_path, plan_path = _parse_generation_args(args)
        ...
        print("source_policy_no_native_text_ocr_transcript_reference_plan_written")
        print(plan_path)
        return 0
    ...
    if check:
        ...
        if current != report:
            print("source_policy_no_native_text_ocr_transcript_reference_plan_report_stale")
            return 1
        print("source_policy_no_native_text_ocr_transcript_reference_plan_report_current")
    else:
        output_path.write_text(report, encoding="utf-8")
        print("source_policy_no_native_text_ocr_transcript_reference_plan_report_written")
    print(
        "can_claim_transcript_reference_plan="
        + _format_bool(summary["can_claim_transcript_reference_plan"])
    )
...
def _parse_generation_args(args: Sequence[str]) -> tuple[Path, Path]:
    if (
        len(args) != 4
        or args[0] != "--from-marker-proxy-eval"
        or args[2] != "--output-transcript-reference-plan"
    ):
        raise ValueError(
            "expected --from-marker-proxy-eval <path> --output-transcript-reference-plan <path>"
        )
...
def _parse_report_args(args: Sequence[str]) -> tuple[Path, Path, bool]:
    ...
    if (
        len(values) != 4
        or values[0] != "--transcript-reference-plan"
        or values[2] != "--output"
    ):
        raise ValueError(
            "expected --transcript-reference-plan <path> --output <path> [--check]"
        )
